...
# these are all the funtions you will need to use 
...
# select which season you wish to project
...
# Also add the weights we will be using

n1_season$weight <- 5
n2_season$weight <- 3
n3_season$weight <- 1

# Create playerID for duplicate names. Eg: There are 2 Sebastian Aho's in 2023 projection
...
# create the transformed versions of each season

season_n1 <- get_info(n1_season)
season_n2 <- get_info(n2_season)
season_n3 <- get_info(n3_season)
...
# create the weighted values for each season

weight_n1 <- n1_joined %>% mutate_at(vars(c(
                      "gp","tot.toi", "g", "a", "p", "ppg",
...
# Add all 3 of seasons together to create a single weighted value for each stat
...
#Eg: A F with w_toi of 12500 will have their stat's regressed 6% towards league average while a F with 3000 will be regressed 25%
                  
f = 750
d = 1000 

# to calc the non-age adjusted projections we have the following formula. If we were projecting goals it would be:
# (regressed goals + weighted goals)/(regressed toi + weighted toi)
# This number is then multipled by GP and proj toi
# Matthews goals would be: ((113/7724)*750+470)/(12357+750)*82*19.62
...
proj_plyr <- sum_plyr %>% transmute(
                ...
                pos= pos,
                proj_toi = proj_toi,
                proj_g =  round((((sum_lg_avg_g/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_g)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2),
                proj_a =  round((((sum_lg_avg_a/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_a)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2),
                proj_p =  round((((sum_lg_avg_pts/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_p)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2),
                proj_ppa =  round((((sum_lg_avg_ppa/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_ppa)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2),
                proj_ppg =  round((((sum_lg_avg_ppg/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_ppg)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2),
                proj_ppp = proj_ppg+proj_ppa,
                proj_sha =  round((((sum_lg_avg_sha/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_sha)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2),
                proj_shg =  round((((sum_lg_avg_shg/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_shg)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2),
                proj_shp = proj_shg+proj_sha,
                proj_sog =  round((((sum_lg_avg_sog/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_sog)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2),
                proj_hit =  round((((sum_lg_avg_hit/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_hit)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2),
                proj_blk =  round((((sum_lg_avg_blk/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_blk)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2),
                proj_fow =  round((((sum_lg_avg_fow/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_fow)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2),
                proj_fol =  round((((sum_lg_avg_fol/sum_lg_avg_toi)*ifelse(pos == "F",f,d))+sum_fol)/(ifelse(pos == "F",f,d)+sum_wtoi)*82*proj_toi,2)
                )
...
proj_stat_age <- proj_plyr %>% mutate_at(
                ...
                (28 - age)*0.0075, (28-age)*0.005)+1))
                
# this will produce a final cleaned up df of the projections                
