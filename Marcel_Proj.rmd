...
# Using case_when we can create GP projections based on specific coniditons. We also will add all 3 of seasons together to create a single weighted value for each stat (both the league average stats and player)
...
sum_plyr <- joined_seasons %>%
            ...
            #1. GP proj for depth F, 2 seasons
                gp.x <= 0.5*max(gp.x) & gp.y <= 0.5*max(gp.y) &                          pos == "F" & proj_toi <= 13.5 ~
                round(35*.6+.2*(gp.x/max(gp.x)*82)
                +.2*(gp.y/max(gp.y)*82)),
            #2. GP proj for depth D, 2 seasons
                gp.x <= 0.5*max(gp.x) & gp.y <= 0.5*max(gp.y) &                          pos == "D" & proj_toi <= 16.5 ~
                round(35*.6+.2*(gp.x/max(gp.x)*82)
                +.2*(gp.y/max(gp.y)*82)),
            #3. GP proj for non depth F, 2 seasons
                gp.x > 0.5*max(gp.x) & gp.y > 0.5*max(gp.y) & pos == "F"~
                round(76*.5+.25*(gp.x/max(gp.x))*82
                +.25*(gp.y/max(gp.y))*82),
            #4. GP proj for non depth D, 2 seasons
                gp.x > 0.5*max(gp.x) & gp.y > 0.5*max(gp.y) & pos == "D"~
                round(76*.5+.25*(gp.x/max(gp.x))*82
                +.25*(gp.y/max(gp.y))*82),
            #5.  GP proj for non depth F, n1 szn < 41, n2 szn > 41
                gp.x <= 0.5*max(gp.x) & gp.y > 0.5*max(gp.y) & pos == "F" 
                & proj_toi > 13.50 ~
                round(74*.7+.125*(gp.x/max(gp.x))*82
                +.175*(gp.y/max(gp.y))*82),
            #6. GP proj for non depth D, n1 szn < 41, n2 szn > 41
                gp.x <= 0.5*max(gp.x) & gp.y > 0.5*max(gp.y) & pos == "D"
                & proj_toi > 16.50 ~
                round(74*.7+.125*(gp.x/max(gp.x))*82
                +.175*(gp.y/max(gp.y))*82),
            #7. GP proj for non depth F, n1 szn > 41, n2 szn < 41
                gp.x > 0.5*max(gp.x) & gp.y <= 0.5*max(gp.y) & pos == "F"
                & proj_toi > 13.5 ~
                round(74*.7+.175*(gp.x/max(gp.x))*82
                +.125*(gp.y/max(gp.y))*82),
            ...
            #11.GP proj for non depth F, 2 seasons
                gp.x <= 0.5*max(gp.x) & gp.y <= 0.5*max(gp.y) &                          pos == "F" & proj_toi > 13.5 ~
                round(74*.8+.10*(gp.x/max(gp.x)*82)
                +.10*(gp.y/max(gp.y)*82)),
            ...
            #13.GP proj for depth F, n1 szn < 41, n2 szn > 41
                gp.x <= 0.5*max(gp.x) & gp.y > 0.5*max(gp.y) & pos == "F" 
                & proj_toi <= 13.50 ~
                round(50*.7+.125*(gp.x/max(gp.x))*82
                +.175*(gp.y/max(gp.y))*82),
            ...
            #15.GP proj for depth F, n1 szn > 41, n2 szn < 41
                gp.x > 0.5*max(gp.x) & gp.y <= 0.5*max(gp.y) & pos == "F"
                & proj_toi <= 13.5 ~
                round(50*.7+.175*(gp.x/max(gp.x))*82
                +.125*(gp.y/max(gp.y))*82),
            #16.GP proj depth D, n1 szn > 41, n2 szn < 41
                gp.x > 0.5*max(gp.x) & gp.y <= 0.5*max(gp.y) & pos == "D"
                & proj_toi <= 16.5~
                round(50*.7+.175*(gp.x/max(gp.x))*82
                +.125*(gp.y/max(gp.y))*82),
            ...
            #18.GP proj for depth D, n1 szn < 41, n2 szn > 41
                ...
                round(50*.7+.125*(gp.x/max(gp.x))*82
                +.175*(gp.y/max(gp.y))*82),
            #19.GP proj for depth F, 2 seasons < 41
                gp.x <= 0.5*max(gp.x) & gp.y <= 0.5*max(gp.y) &                          pos == "F" & proj_toi <= 13.5 ~
                round(50*.8+.10*(gp.x/max(gp.x)*82)
                +.10*(gp.y/max(gp.y)*82)),
            #20.GP proj for depth D, 2 seasons
                gp.x <= 0.5*max(gp.x) & gp.y <= 0.5*max(gp.y) &                          pos == "D" & proj_toi <= 16.5 ~
                round(50*.8+.10*(gp.x/max(gp.x)*82)
                +.10*(gp.y/max(gp.y)*82))),
...
# [(regressed g + weighted g)/(regressed toi + weighted toi)]*GP*toi
# regressed goals is how many goals a lg average player would've scored in that players TOI
# This number is then multipled by GP and toi
# Eg: Matthews 2023 goals would be: ((128/8690)*750+530)/(13862+750)*75*20.15
